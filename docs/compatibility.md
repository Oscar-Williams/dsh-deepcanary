...
DeepCanary records the current source candidate, the published RC1 result, historical releases, and distribution-package checks separately. This keeps each identity reproducible while compatibility with DSH advances:

1. **0.1.1-rc.2 engineering candidate** — the current `main` source, built `lib/`, alpha.5 public session APIs, reconciliation tests, and fresh local package identity.
...
`0.1.1-rc.2` is the current source-development identity and `0.1.1-rc.1` remains the required published-artifact baseline for new installation and compatibility tests. The RC4 and RC2 results remain available for comparison. The alpha.5 RC1 evidence is recorded in [`benchmark/alpha5-compatibility-receipt.json`](../benchmark/alpha5-compatibility-receipt.json); the RC4 evidence remains in [`benchmark/release-candidate-receipt.json`](../benchmark/release-candidate-receipt.json). The earlier alpha.3 browser record remains in [`benchmark/alpha3-compatibility-receipt.json`](../benchmark/alpha3-compatibility-receipt.json).
...
| Component | 0.1.1-rc.2 engineering candidate | 0.1.1-rc.1 published prerelease | RC.4 / historical RC.2 |
| --- | --- | --- | --- |
...
| Plugin | `main` source build, local RC2 tarball; publication pending fresh gates | `v0.1.1-rc.1` GitHub tag, Release asset, and npm `next` | Keep each plugin revision and DSH runtime in the same acceptance record |
...
| `ctx.sessions.list()` / `Session.snapshotEvents()` | supplied by alpha.5 Session service | RC2 authoritative startup reconciliation, Human Needed reconstruction, orphan grace, and exact event-sequence metadata | Reconciliation status remains `unavailable` when the host composition omits the public list surface |
...
The Web UI uses the DSH alpha.5 client-module interfaces: the plugin manifest exposes `dsh.client` and `./client`, and the host provides the `sidebar.footer.action`, `shell.overlay`, and `settings.plugin.item` slots. The state response also exposes the RC2 reconciliation phase, epoch, authoritative flag, verification flag, and bounded event counts. A profile with the historical `v0.1.0-rc.1` package does not provide the current four interaction gates or the standard settings card.

For `0.1.1-rc.1` verification, use either the locally generated `dsh-deepcanary-0.1.1-rc.1.tgz`, the immutable GitHub tag/Release asset, or npm `next`. For the current RC2 engineering candidate, build and pack from `main`, then bind the local tarball hash to the fresh Gate report. Verify `dsh --profile web --dump-config`, the DeepCanary health, state/reconciliation, and OutcomeReceipt routes, the nine registered tools, and the client-module boot graph in a fresh isolated profile. The alpha.5 receipt records the RC1 runtime, profiles, package digest, public-tag installation, npm metadata, and regression result. The RC4 and historical RC2 installation commands remain tied to their original runtimes and are retained for reproduction.

## Known limitations

- RC2 historical evidence is tied to the alpha.2 source tag. RC4 evidence is tied to the alpha.4 tag and its exact local package/profile. Published RC1 evidence is tied to the alpha.5 tag and the exact package/profile recorded by the immutable receipt. RC2 engineering evidence is tied to the current source commit and fresh local tarball digest.
...
- The Persistent Supervisor prototype writes a bounded snapshot and short-lived lease, exposes read-only diagnostics, retries while a fresh lease is held by another instance, and protects against competing owners. Its snapshot includes versioned dedupe hashes, interrupt-budget timestamps, and a bounded logical browser delivery ledger with idempotent attempt transitions. The prototype is experimental and off by default; Windows OS-visible evidence still requires a real observation bound to that attempt. RC2 adds the first public alpha.5 session-list reconciliation slice, startup Human Needed reconstruction, previously observed-session disposal convergence, and a timestamp-only orphan grace path; full restart acceptance, OS delivery-state convergence, cross-process orphan handling, and soak acceptance remain Gate E work.
