...
export const SUPERVISOR_POLICY_STATE_SCHEMA_VERSION = 1;
...
function isPolicyState(value) {
    if (!isRecord(value)
        ...
        || !Array.isArray(value.interruptConsumedAt))
        return false;
    ...
    if (dedupe.length > 512 || !dedupe.every(entry => isRecord(entry)
        ...
        && isIsoDate(entry.acceptedAt)))
        return false;
    if (value.interruptConsumedAt.length > 256 || !value.interruptConsumedAt.every(isIsoDate))
        return false;
    return true;
}
function clonePolicyState(policyState) {
...
function cloneSnapshot(snapshot) {
    return {
        ...
        ...(snapshot.policyState === undefined ? {} : { policyState: clonePolicyState(snapshot.policyState) }),
...
function isDeliveryEntry(value) {
...
function emptySnapshot(runtimeVersion, now) {
    return {
        schemaVersion: SUPERVISOR_SCHEMA_VERSION,
        revision: 0,
...
function normalizeSnapshot(value, runtimeVersion, now, maxSessions, maxPending) {
    if (!isRecord(value) || value.schemaVersion !== SUPERVISOR_SCHEMA_VERSION)
        return undefined;
    const host = value.host;
    if (!isRecord(host) || !isSafeRef(host.runtimeVersion) || !isHostStatus(host.status))
        return undefined;
    ...
    const sessions = Array.isArray(value.sessions)
        ? value.sessions.flatMap(candidate => {
            if (!isRecord(candidate) || !isSafeRef(candidate.sessionRef) || !isAttentionLevel(candidate.attentionLevel))
                return [];
    ...
    return {
        ...
        sessions,
        pending,
        ...(policyState === undefined ? {} : { policyState: clonePolicyState(policyState) }),
...
export class SupervisorStore {
    ...
    async load(runtimeVersion, now, maxSessions = DEFAULT_MAX_SESSIONS, maxPending = DEFAULT_MAX_PENDING) {
        try {
            let raw;
            try {
                raw = JSON.parse(await readFile(this.snapshotFile, 'utf8'));
            }
            catch (error) {
     ...
     * lock.
     */
    async saveOwned(snapshot, expected) {
        return this.withOperationLock(async () => {
            const current = await this.loadLeaseUnlocked();
            if (!sameLease(current, expected))
                return undefined;
            const payload = { schemaVersion: SUPERVISOR_SCHEMA_VERSION, snapshot: cloneSnapshot(snapshot) };
    ...
    async loadLeaseUnlocked() {
        try {
            let raw;
            try {
                raw = JSON.parse(await readFile(this.leaseFile, 'utf8'));
            }
            catch (error) {
                if (isNodeError(error, 'ENOENT'))
                    return undefined;
    ...
    async replaceLease(expected, next) {
        return this.withOperationLock(async () => {
            const current = await this.loadLeaseUnlocked();
            if (!sameLease(current, expected))
                return false;
    ...
    async archiveStaleLease(expected, now) {
        return this.withOperationLock(async () => {
            const current = await this.loadLeaseUnlocked();
            if (!sameLease(current, expected))
                return false;
        ...
        try {
            ...
            for (const entry of entries) {
                if (!entry.isFile())
                    continue;
                try {
                    total += (await stat(path.join(this.directory, entry.name))).size;
                }
    ...
    async quarantine(file, now, reason) {
        ...
        while (true) {
            try {
                ...
                if (Date.now() >= deadline)
                    throw new Error('supervisor operation lock timeout');
...
export class PersistentSupervisor {
    ...
    async start() {
        if (this.lifecycle === 'running' || this.lifecycle === 'degraded')
        ...
        try {
            ...
            if (this.stopping || this.disposed || this.lease === undefined)
                return false;
    ...
    update(snapshot) {
        ...
        if (normalized === undefined)
            return;
        if (normalized.revision < this.snapshotValue.revision)
            return;
        this.snapshotValue = normalized;
        this.lastSnapshotAt = normalized.generatedAt;
        this.wakeCount += 1;
        if (this.lease !== undefined && (this.lifecycle === 'running' || this.lifecycle === 'degraded'))
    ...
    async stop() {
        if (this.disposed || this.lifecycle === 'stopped')
            return;
        if (this.stopping) {
            await this.saveChain;
            return;
        ...
        try {
            ...
            if (this.lease !== undefined) {
                try {
                    await this.store.releaseLease(this.lease);
                }
    ...
    scheduleStandbyRetry() {
        if (this.heartbeatTimer !== undefined)
            clearInterval(this.heartbeatTimer);
        this.heartbeatTimer = undefined;
        if (this.disposed || this.stopping || this.standbyTimer !== undefined)
            return;
        this.standbyTimer = setTimeout(() => {
            this.standbyTimer = undefined;
            if (!this.disposed && !this.stopping && this.lifecycle === 'standby')
                void this.start();
    ...
    async persistNow() {
        if (this.lease === undefined || this.disposed)
            return;
        const snapshot = cloneSnapshot(this.snapshotValue);
        this.saveChain = this.saveChain
            .then(async () => {
            const lease = this.lease;
            if (lease === undefined || this.disposed)
                return;
    ...
    async heartbeat() {
        if (this.lease === undefined || this.disposed || this.stopping)
            return;
        ...
        this.saveChain = this.saveChain.then(async () => {
            ...
            const next = { ...this.lease, heartbeatAt };
...
export function supervisorSnapshotFor(runtimeVersion, hostStatus, revision, sessions, pending, now = Date.now(), maxPending = DEFAULT_MAX_PENDING, policyState, deliveryLedger) {
    return {
        schemaVersion: SUPERVISOR_SCHEMA_VERSION,
        ...
        ...(policyState === undefined || !isPolicyState(policyState) ? {} : { policyState: clonePolicyState(policyState) }),
