...
async function actionHandler(req, res, service) {
    ...
    try {
        ...
        if (action === 'acknowledge')
            updated = service.acknowledge(id);
        else if (action === 'mute')
            updated = service.mute(id);
        else if (action === 'snooze')
            updated = service.snooze(id, typeof payload.minutes === 'number' ? payload.minutes : 30);
        else if (action === 'feedback')
            updated = service.feedback(id, payload.useful === true, typeof payload.note === 'string' ? payload.note : undefined);
        else if (action === 'jump') {
...
export function installWebRoutes(ctx, service) {
    ...
    context.inject?.(['webServer'], (webCtx) => {
        ...
        const disposers = [
            ...
            server.register({ kind: 'exact', path: `${basePath}/health`, handler: (req, res) => {
                    if (req.method !== 'GET')
                        ...
                        sendJson(res, 200, { ok: true, plugin: service.status().plugin, sessions: service.status().sessions, tools: service.status().tools });
                } }),
        ...
        webCtx.on?.('webserver/index-inject', (rows) => {
