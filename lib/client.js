window.__ModuleLoader__.load({
  ...
  async function notify(snapshot, t, controller) {
    ...
    for (const item of snapshot.inbox.filter((value) => {
      ...
      return (value.action === "INTERRUPT" || value.action === "ESCALATE") && !attempted.has(value.id) && (!Number.isFinite(mutedUntil) || mutedUntil <= Date.now());
    }).slice(0, 3)) {
      const titleKey = notificationTitleKey(item);
      if (titleKey === void 0) continue;
      const navigationUrl = item.sessionId ? `/?session=${encodeURIComponent(item.sessionId)}` : void 0;
      ...
      const notificationAttemptId = delivery === void 0 ? void 0 : await opaqueRef(`${item.id}:attempt:${Date.now()}:${Math.random()}`);
  ...
  function itemCard(item, state, t, controller, selected) {
    ...
    const actions = [
      item.sessionId ? actionButton(translate(t, `item.open.${item.level}`), () => {
