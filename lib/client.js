window.__ModuleLoader__.load({
  ...
  function notify(snapshot, t, controller) {
    ...
    for (const item of snapshot.inbox.filter((value) => {
      ...
      return (value.action === "INTERRUPT" || value.action === "ESCALATE") && !seen.has(value.id) && (!Number.isFinite(mutedUntil) || mutedUntil <= Date.now());
    }).slice(0, 3)) {
      const navigationUrl = item.sessionId ? `/?session=${encodeURIComponent(item.sessionId)}` : void 0;
      const notification = new Notification(`DeepCanary \xB7 ${notificationTitle(item, t)}`, {
