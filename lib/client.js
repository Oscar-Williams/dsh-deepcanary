window.__ModuleLoader__.load({
  ...
  function handleNotificationClick(notification, id, focus, open, openSession, navigate, url, sessionId) {
    focus();
    const hasNativeSessionTarget = sessionId !== void 0 && openSession !== void 0;
    const openedSession = sessionId !== void 0 && openSession !== void 0 ? openSession(sessionId) : false;
    open(id);
    if (!hasNativeSessionTarget && !openedSession && navigate !== void 0 && url !== void 0) navigate(url);
    notification.close();
  }
  ...
  var ATTEMPTED_NOTIFICATION_KEY = "dsh-deepcanary-notification-attempted";
  var LEGACY_SEEN_KEY = "dsh-deepcanary-notified";
  var AUTO_OPEN_KEY = "dsh-deepcanary-auto-opened";
  var MIN_WIDTH = 320;
  var MAX_WIDTH = 640;
  var MIN_HEIGHT = 320;
  var MAX_HEIGHT = 720;
  var AUTO_OPEN_REASONS = /* @__PURE__ */ new Set(["HOST_UNREACHABLE", "SUBAGENT_PRESSURE", "HUMAN_APPROVAL_REQUIRED", "HUMAN_QUESTION_PENDING"]);
  var zh = {
    "trigger.open": "\u6253\u5F00 DeepCanary",
    "trigger.close": "\u5173\u95ED DeepCanary",
    "trigger.label": "DeepCanary \u4EFB\u52A1\u63D0\u9192",
    "trigger.count": "{count} \u6761\u5F85\u5904\u7406\u63D0\u9192",
    "panel.title": "DeepCanary",
    "panel.subtitle": "\u4EFB\u52A1\u63D0\u9192\u4E0E\u52A8\u6001",
    ...
    "panel.emptyHint": "\u9700\u8981\u4F60\u5173\u6CE8\u7684\u4EFB\u52A1\u52A8\u6001\u4F1A\u663E\u793A\u5728\u8FD9\u91CC\u3002",
    ...
    "panel.settings": "\u63D0\u9192\u8BBE\u7F6E",
    "panel.settingsHint": "\u5728 DSH \u8BBE\u7F6E > \u63D2\u4EF6 > DeepCanary \u4E2D\u8C03\u6574\u63D0\u9192\u7B56\u7565\u3002",
    ...
    "panel.policyHint": "C1 \u5EFA\u8BAE\u67E5\u770B \xB7 C2 \u9700\u8981\u5904\u7406 \xB7 C3 \u7ACB\u5373\u5904\u7406",
    "panel.delivery": "\u901A\u77E5\u9884\u7B97 {used}/{limit}\uFF0C\u5269\u4F59 {remaining} \xB7 \u53BB\u91CD {dedupe} \u5206\u949F \xB7 \u5408\u5E76 {bundle} \u79D2 \xB7 {quiet}",
    ...
    "settings.description": "\u8BBE\u7F6E\u54EA\u4E9B\u4EFB\u52A1\u52A8\u6001\u9700\u8981\u63D0\u9192\uFF0C\u4EE5\u53CA\u63D0\u9192\u7684\u9891\u7387\u3002",
    "settings.hint": "\u8BBE\u7F6E\u4FDD\u5B58\u5728\u5F53\u524D DSH \u914D\u7F6E\u4E2D\uFF0C\u4E0D\u4F1A\u4FEE\u6539\u4F1A\u8BDD\u5185\u5BB9\u3002",
    ...
    "item.reason.HOST_SUSPECTED_STALL": "\u5DF2\u8FDE\u7EED {idleMinutes} \u5206\u949F\u672A\u89C2\u5BDF\u5230\u4F1A\u8BDD\u6D3B\u52A8\uFF0C\u4EFB\u52A1\u53EF\u80FD\u4ECD\u5728\u8FD0\u884C\u3002",
    ...
    "item.level.C3": "\u7ACB\u5373\u5904\u7406",
    "item.level.C0": "\u8BB0\u5F55",
    "item.levelLabel": "{level} \xB7 {label}",
    "item.open.C1": "\u67E5\u770B DSH",
    ...
    "notification.title.HOST_SUSPECTED_STALL": "\u4F1A\u8BDD\u53EF\u80FD\u505C\u6EDE\uFF0C\u8BF7\u68C0\u67E5",
  ...
  var en = {
    ...
    "trigger.label": "DeepCanary task alerts",
    "trigger.count": "{count} pending alerts",
    "panel.title": "DeepCanary",
    "panel.subtitle": "Task alerts and updates",
    ...
    "panel.emptyHint": "Task updates that need your attention will appear here.",
    ...
    "panel.settingsHint": "Adjust alerts in DSH Settings > Plugins > DeepCanary.",
    ...
    "panel.policyHint": "C1 Review \xB7 C2 Action needed \xB7 C3 Act now",
    "panel.delivery": "Interrupt budget {used}/{limit}, {remaining} remaining \xB7 dedupe {dedupe}m \xB7 bundle {bundle}s \xB7 {quiet}",
    ...
    "settings.description": "Choose which task updates need a reminder and how often to notify you.",
    "settings.hint": "Settings are saved in your current DSH configuration and do not change session content.",
    ...
    "item.reason.HOST_SUSPECTED_STALL": "No session activity has been observed for {idleMinutes} minutes. The task may still be running.",
    "item.reason.TOOL_FAILURE_LOOP": "Tool \u201C{toolName}\u201D has failed {failureCount} times in a row.",
    "item.reason.NO_MEANINGFUL_PROGRESS": "The session is running, but meaningful progress has not been observed yet.",
    ...
    "notification.title.HOST_SUSPECTED_STALL": "Possible session stall \u2014 check the task",
  ...
  async function opaqueRef(value) {
  ...
  function createController(sessions) {
    ...
    const nextPollDelay = () => {
      const base = document.visibilityState === "hidden" ? 6e4 : 15e3;
      return Math.min(12e4, base * 2 ** Math.min(failureCount, 3));
    };
    const scheduleFromSessionRevision = () => {
      schedule(document.visibilityState === "hidden" ? 1e3 : 250);
    };
    const controller = {
      getState: () => state,
      subscribe: (listener) => {
      ...
      dispose: () => {
        ...
        sessionFeedSubscription = void 0;
      ...
      refresh: async () => {
        ...
        try {
          ...
          if (requestTimeout !== void 0) window.clearTimeout(requestTimeout);
          inFlight = false;
      ...
      claimDelivery: async (id, expiresAt) => {
        try {
          ...
          return outcome === "granted" || outcome === "already-claimed" || outcome === "already-complete" || outcome === "unavailable" ? outcome : "unavailable";
        } catch {
          return "unavailable";
        }
      },
      ...
      jump: async (id) => {
        ...
        try {
          ...
          if (result.available && result.sessionId !== void 0 && controller.openSession(result.sessionId)) {
            controller.close();
            return;
          }
  ...
  function levelLabel(level, t) {
    const key = level === "C1" ? "item.level.C1" : level === "C2" ? "item.level.C2" : level === "C3" ? "item.level.C3" : "item.level.C0";
    return translate(t, key);
  }
  function notificationTitle(item, t) {
    const key = notificationTitleKeys[item.reasonCode];
    const title = key === void 0 ? translate(t, "item.unknownReason", { code: item.reasonCode }) : translate(t, key, item.messageParams);
    return `${translate(t, "item.levelLabel", { level: item.level, label: levelLabel(item.level, t) })} \xB7 ${title}`;
  }
  function notificationTitleKey(item) {
    const key = notificationTitleKeys[item.reasonCode];
    return key === void 0 ? void 0 : key;
  }
  function reasonCodeText(reasonCode, t) {
    const key = reasonKeys[reasonCode];
    return key === void 0 ? translate(t, "item.unknownReason", { code: reasonCode }) : translate(t, key);
  }
  function notificationBody(item, t) {
    const parts = [reasonText(item, t)];
    if (item.bundleCount > 1) {
      const relatedReasons = item.reasonCodes.filter((reasonCode) => reasonCode !== item.reasonCode).slice(0, 2).map((reasonCode) => reasonCodeText(reasonCode, t));
      parts.push(relatedReasons.length > 0 ? translate(t, "notification.bundleDetail", { count: item.bundleCount, reasons: relatedReasons.join(chineseLocale() ? "\u3001" : ", ") }) : translate(t, "notification.bundle", { count: item.bundleCount }));
    }
    return parts.join(" ");
  }
  ...
  function suppressedTypeText(reasonCode, t) {
    const key = reasonKeys[reasonCode];
    return key === void 0 ? reasonCode : translate(t, key);
  }
  function deliveryText(snapshot, t) {
    const delivery = snapshot.status.delivery;
    const quiet = delivery.quietHours.enabled ? translate(t, delivery.quietHours.active ? "panel.quiet.active" : "panel.quiet.inactive", delivery.quietHours) : translate(t, "panel.quiet.disabled");
  ...
  async function notify(snapshot, t, controller) {
    ...
    try {
      const current = JSON.parse(window.localStorage.getItem(ATTEMPTED_NOTIFICATION_KEY) ?? "[]");
      const legacy = JSON.parse(window.localStorage.getItem(LEGACY_SEEN_KEY) ?? "[]");
      attempted = /* @__PURE__ */ new Set([
        ...Array.isArray(current) ? current.filter((value) => typeof value === "string") : [],
        ...Array.isArray(legacy) ? legacy.filter((value) => typeof value === "string") : []
      ]);
    } catch {
      attempted = /* @__PURE__ */ new Set();
    }
    for (const item of snapshot.inbox.filter((value) => {
      const mutedUntil = value.mutedUntil === void 0 ? Number.NaN : Date.parse(value.mutedUntil);
      return (value.action === "INTERRUPT" || value.action === "ESCALATE") && (!Number.isFinite(mutedUntil) || mutedUntil <= Date.now()) && !attempted.has(value.id);
    }).slice(0, 3)) {
      const titleKey = notificationTitleKey(item);
      if (titleKey === void 0) continue;
      const navigationUrl = item.sessionId && item.targetAvailable !== false ? `/?session=${encodeURIComponent(item.sessionId)}` : void 0;
      ...
      const claim = await controller.claimDelivery(item.id, new Date(Date.now() + 3e4).toISOString());
      ...
      let notificationAttemptId;
      if (delivery !== void 0) {
        try {
          notificationAttemptId = await opaqueRef(`${item.id}:attempt:${Date.now()}:${Math.random()}`);
        } catch {
          notificationAttemptId = void 0;
        }
      }
      if (delivery !== void 0 && notificationAttemptId !== void 0) {
      ...
      try {
        const options = {
          ...
          data: {
            itemId: item.id,
            ...delivery === void 0 ? {} : { notificationRef: delivery.notificationRef, tagRef: delivery.tagRef },
            ...navigationUrl === void 0 ? {} : { navigationUrl }
          }
        };
        if (delivery !== void 0) options.tag = delivery.tagRef;
        notification = new Notification(`DeepCanary \xB7 ${notificationTitle(item, t)}`, options);
      } catch {
  ...
  function ResizeHandle(props) {
    ...
    const activePointerId = (0, import_react.useRef)(void 0);
    const handleRef = (0, import_react.useRef)(null);
    const propsRef = (0, import_react.useRef)(props);
    propsRef.current = props;
    const applyPointerResize = (clientX, clientY) => {
    ...
    const finishPointerResize = (pointerId) => {
      ...
      activePointerId.current = void 0;
    ...
    (0, import_react.useEffect)(() => {
      ...
      return () => {
        ...
        if (activePointerId.current !== void 0) finishPointerResize(activePointerId.current);
      };
    }, []);
  ...
  function DeepCanarySettingsCard(props) {
    ...
    return (0, import_react.createElement)(
      ...
      open && (0, import_react.createElement)(
        ...
        field(
          translate(t, "settings.supervisorMode"),
          (0, import_react.createElement)("select", {
            value: current.supervisorMode,
            disabled,
            onChange: (event) => {
              edit("supervisorMode", event.target.value);
            }
          }, ...["off", "experimental"].map((value) => (0, import_react.createElement)("option", { key: value, value }, translate(t, "settings.supervisorMode." + value))))
        ),
  ...
  function decisionTraceDetails(item, t) {
    ...
    const traceRows = [
      (0, import_react.createElement)("p", { key: "why" }, translate(t, "item.decisionWhy", { text: item.why })),
  ...
  function itemCard(item, state, t, controller, selected) {
    ...
    const children = [
      (0, import_react.createElement)(
        ...
        (0, import_react.createElement)("span", { className: "dsc-level-label" }, levelLabel(item.level, t)),
    ...
    if (item.sessionId === void 0) {
      children.push((0, import_react.createElement)(
        "small",
        { className: "dsc-card-suggestion", key: "session-link" },
        translate(t, "item.noSessionLink")
      ));
    } else if (item.targetAvailable === false) {
      children.push((0, import_react.createElement)(
        "small",
        { className: "dsc-card-suggestion", key: "session-ended" },
        translate(t, "item.sessionEnded")
      ));
    }
    if (item.mutedUntil !== void 0 && Number.isFinite(Date.parse(item.mutedUntil)) && Date.parse(item.mutedUntil) > Date.now()) {
      children.push((0, import_react.createElement)(
        "small",
        { className: "dsc-card-suggestion", key: "muted" },
    ...
    if (state.notice?.id === item.id) {
      children.push((0, import_react.createElement)("p", {
        className: `dsc-action-notice dsc-action-notice-${state.notice.tone}`,
        key: "notice",
        role: "status",
        "aria-live": "polite"
      }, noticeText(state.notice, t)));
    } else if (item.feedback !== void 0) {
      children.push((0, import_react.createElement)("p", {
        className: "dsc-action-notice",
        key: "feedback-state",
        role: "status"
      }, item.feedback.value === "wrong-level" ? translate(t, "item.feedbackWrongLevel") : item.feedback.value === "already-resolved" ? translate(t, "item.feedbackAlreadyResolved") : translate(t, item.feedback.useful ? "item.notice.feedbackUseful" : "item.notice.feedbackIrrelevant")));
    }
    const actions = [
      item.sessionId && item.targetAvailable !== false ? actionButton(translate(t, `item.open.${item.level}`), () => {
        void controller.jump(item.id);
      }, busy, true) : actionButton(translate(t, "item.acknowledge"), () => {
    ...
    const mutedUntil = item.mutedUntil === void 0 ? Number.NaN : Date.parse(item.mutedUntil);
    moreActions.push(Number.isFinite(mutedUntil) && mutedUntil > Date.now() ? actionButton(translate(t, "item.unmute"), () => {
      void controller.action(item.id, { action: "unmute" });
    }, busy) : actionButton(translate(t, "item.mute"), () => {
    ...
    moreActions.push(
      (0, import_react.createElement)("span", { className: "dsc-more-label", key: "feedback-label" }, translate(t, "item.feedbackMenu")),
    ...
    actions.push((0, import_react.createElement)(
      "details",
      { className: "dsc-more", key: "more" },
      (0, import_react.createElement)("summary", null, translate(t, "item.moreActions")),
      (0, import_react.createElement)("div", { className: "dsc-more-actions" }, ...moreActions)
    ));
  ...
  function DeepCanaryOverlay(props) {
    ...
    (0, import_react.useEffect)(() => {
      if (state.snapshot !== void 0) void notify(state.snapshot, props.t, props.controller);
    ...
    if (snapshot !== void 0) {
      ...
      body.push((0, import_react.createElement)("p", { className: "dsc-policy-summary", key: "policy-hint" }, translate(props.t, "panel.policyHint")));
      if (snapshot.status.delivery !== void 0) {
        body.push((0, import_react.createElement)("p", { className: "dsc-settings-hint", key: "delivery" }, deliveryText(snapshot, props.t)));
      }
      const suppressedReasonCodes = snapshot.settings.suppressedReasonCodes ?? [];
      body.push((0, import_react.createElement)(
        "div",
        { className: "dsc-suppressed-summary", key: "suppressed" },
        (0, import_react.createElement)("span", null, translate(props.t, "panel.suppressed", {
          types: suppressedReasonCodes.length === 0 ? translate(props.t, "panel.suppressed.none") : suppressedReasonCodes.map((reasonCode) => suppressedTypeText(reasonCode, props.t)).join("\u3001")
        })),
        suppressedReasonCodes.length > 0 && (0, import_react.createElement)(
          "div",
          { className: "dsc-suppressed-actions" },
      ...
      (0, import_react.createElement)(
        ...
        (0, import_react.createElement)(
          ...
          (0, import_react.createElement)(
            ...
            state.notice !== void 0 && " \xB7 " + noticeText(state.notice, props.t)
          ),
          (0, import_react.createElement)("button", {
            className: "dsc-toolbar-button",
            type: "button",
          ...
          }, !notificationSupported ? translate(props.t, "panel.notification.unavailable") : notificationBlocked ? translate(props.t, "panel.notification.denied") : notificationPermission === "granted" ? translate(props.t, "panel.notification.enabled") : translate(props.t, "panel.notification.enable"))
        ),
