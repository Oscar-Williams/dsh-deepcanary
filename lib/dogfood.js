...
const taskOrigins = new Set(['natural', 'controlled', 'replay', 'unknown']);
const reviewSources = new Set(['user-feedback', 'engineering-review', 'independent-audit', 'unknown']);
const reviewBases = new Set(['explicit-user-feedback', 'direct-user-observation', 'runtime-ledger', 'independent-session-audit', 'policy-expectation', 'unknown']);
const reviewConfidences = new Set(['high', 'medium', 'low', 'unknown']);
const visibilityStatuses = new Set(['visible', 'unknown']);
const visibilitySources = new Set(['inbox-materialized', 'browser-constructed', 'os-observed', 'user-confirmed', 'unknown']);
...
function isDogfoodRun(value) {
    return isRecord(value)
        ...
        && (value.taskOrigin === undefined || taskOrigins.has(value.taskOrigin))
...
function isDogfoodObservation(value, runId) {
    ...
    if (value.userFeedback !== undefined && !userFeedbackValues.has(value.userFeedback))
        return false;
    if (value.reviewSource !== undefined && !reviewSources.has(value.reviewSource))
        return false;
    if (value.reviewBasis !== undefined && !reviewBases.has(value.reviewBasis))
        return false;
    if (value.reviewConfidence !== undefined && !reviewConfidences.has(value.reviewConfidence))
        return false;
    const reviewMetadata = [value.reviewSource, value.reviewBasis, value.reviewConfidence].filter(candidate => candidate !== undefined);
    if (reviewMetadata.length !== 0 && reviewMetadata.length !== 3)
        return false;
    ...
    if (value.deliveryVisibility !== undefined) {
        if (!isRecord(value.deliveryVisibility)
            || !visibilityStatuses.has(value.deliveryVisibility.status)
            || !visibilitySources.has(value.deliveryVisibility.source))
            return false;
    }
...
export function hasDogfoodReview(observation) {
 ...
 */
export function hasQualifiedDogfoodReview(observation) {
...
export function isUserFacingDeliveryObservation(observation) {
...
function uniqueFinalDeliveryUnits(observations) {
    const units = new Map();
    for (const observation of observations) {
        if (!isUserFacingDeliveryObservation(observation) || observation.deliveryUnitRef === undefined)
            continue;
