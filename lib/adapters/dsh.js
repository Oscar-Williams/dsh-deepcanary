...
export class ContextDshAdapter {
    ...
    async performReconcile() {
        ...
        try {
            ...
            for (const { snapshot } of authoritative.values())
...
function snapshotFromSession(value, active) {
    ...
    for (const event of events ?? []) {
        ...
        if (time !== undefined && isMeaningfulSessionEvent(type, data))
            lastMeaningfulAt = time;
        if (type === 'turn/start') {
            running = true;
            turnState = 'running';
        ...
        if (type === 'tool/call') {
            if (callId === undefined || !activeTools.has(callId))
                activeToolCount += 1;
            if (callId !== undefined)
                activeTools.set(callId, observedToolName);
            toolClass = toolClassForName(observedToolName);
            if (observedToolName !== undefined)
                lastToolName = observedToolName;
        }
        if (type === 'tool/result') {
            if (callId === undefined || activeTools.delete(callId))
...
function applyEventToSnapshot(snapshots, event) {
    ...
    if (snapshot !== undefined && event.type === 'session/event' && isRecord(event.event)) {
        ...
        if (occurredAt !== undefined && isMeaningfulSessionEvent(typeof event.event.type === 'string' ? event.event.type : '', isRecord(event.event.data) ? event.event.data : {}))
