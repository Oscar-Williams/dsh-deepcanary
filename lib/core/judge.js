...
function levelFor(signal) {
    ...
    if (signal.kind === 'HOST_UNREACHABLE')
        return signal.evidence.some(item => item.authority === 'host' || item.authority === 'runtime') ? 'C3' : 'C2';
    if (signal.kind === 'SUBAGENT_PRESSURE') {
        if (signal.severityHint === 3)
            return 'C3';
        if (signal.severityHint === 2)
            return 'C2';
        return 'C1';
    }
...
function messageParamsFor(signal) {
    const params = {};
    for (const key of ['threshold', 'failureCount', 'activeSubagents', 'idleMs']) {
