...
export function dogfoodRunFromEnvironment(pluginVersion, defaultRuntimeTag, startedAt = new Date().toISOString()) {
    ...
    const taskOrigin = value(DOGFOOD_ENVIRONMENT.taskOrigin);
...
function usefulnessPatch(useful, value) {
    const common = {
        ...
        deliveryVisibility: { status: 'visible', source: 'user-confirmed' },
    };
    if (useful)
        return { ...common, reviewLabel: 'correct-useful', policyReview: 'correct', userFeedback: 'useful', usefulnessReason: 'actionable' };
    if (value === 'already-resolved')
        return { ...common, reviewLabel: 'already-resolved', policyReview: 'too-late', userFeedback: 'not-useful', usefulnessReason: 'already-resolved' };
    if (value === 'wrong-level')
        return { ...common, reviewLabel: 'wrong-level', policyReview: 'wrong-level', userFeedback: 'not-useful', usefulnessReason: 'wrong-level' };
...
export class DogfoodRuntimeRecorder {
    ...
    record(input) {
        ...
        const observation = {
            ...
            ...(input.deliveryUnit === undefined || !['inbox', 'digest', 'interrupt', 'escalate'].includes(input.disposition) ? {} : { deliveryVisibility: { status: 'unknown', source: 'unknown' } }),
        };
