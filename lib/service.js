...
import { signalFromAgentError, signalFromHostRecovery, signalFromHostProbe, signalFromStall, signalFromStallRecovery, signalFromSubagentPressure, signalFromAuthoritativeHumanWait, signalsFromSessionEvent, } from './providers.js';
...
export class DeepCanaryService {
    ...
    reconcileOrphans(now) {
        ...
        let changed = false;
        for (const item of this.items) {
            if (!isActiveInboxStatus(item.status) || item.sessionId === undefined)
                continue;
