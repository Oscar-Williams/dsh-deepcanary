...
import { signalFromAgentError, signalFromHostRecovery, signalFromHostProbe, signalFromStall, signalFromStallRecovery, signalFromSubagentPressure, signalFromAuthoritativeHumanWait, isMeaningfulSessionEvent, toolClassForName, toolCallIdOf, signalsFromSessionEvent, } from './providers.js';
...
const PLUGIN_VERSION = '0.1.1-rc.5';
...
function isActiveInboxStatus(status) {
    return status === 'open' || status === 'seen' || status === 'snoozed' || status === 'muted';
}
function taskStateForReason(reasonCode) {
    switch (reasonCode) {
...
export class DeepCanaryService {
    ...
    dedupe;
    budget;
    deliveryLedger = new DeliveryLedger();
    ...
    start() {
        ...
        this.adapterSubscription = this.adapter.subscribe(event => {
            if (event.type === 'session/created')
                this.onSessionCreated(event.session, event.snapshot);
            else if (event.type === 'session/event')
                this.onSessionEvent(event.session, event.event, event.snapshot);
            else
    ...
    claimNotification(id, payload) {
        const item = this.find(id);
        if (item === undefined || (item.action !== 'INTERRUPT' && item.action !== 'ESCALATE'))
            return 'unavailable';
        const result = this.deliveryLedger.claim({
            verdictId: item.id,
            conditionGeneration: item.bundleKey ?? item.id,
            sink: 'browser',
            clientInstance: payload.notificationClientInstance,
            expiresAt: payload.notificationExpiresAt,
        });
        this.syncSupervisor();
    ...
    isNotificationClaimPayload(payload) {
    ...
    ensureSupervisorStarted() {
        if (this.supervisorStarted || this.config.supervisorMode !== 'experimental' || this.disposed)
            return;
        this.supervisorStarted = true;
        this.supervisorStart = this.startSupervisor();
    }
    syncSupervisor() {
        if (this.config.supervisorMode !== 'experimental')
            return;
        const now = Date.now();
        ...
        this.supervisor.update(supervisorSnapshotFor(this.adapter.hostVersion, this.supervisorHostStatus, this.revision, sessions, pending.map(item => hashMetadata(item.id)), now, this.config.maxInboxItems, this.supervisorPolicyState(now), this.deliveryLedger.snapshot()));
    }
    ...
    onSessionCreated(value, authoritativeSnapshot) {
        ...
        if (authoritativeSnapshot?.eventCount !== undefined && authoritativeSnapshot.waitingForHuman === false
            ...
            void this.ready.then(() => {
                if (!this.disposed)
                    this.resolveHumanWait(id, now, authoritativeSnapshot.turnState === 'running' ? 'running' : 'unknown');
            });
        }
        if (authoritativeSnapshot?.waitingForHuman && authoritativeSnapshot.humanNeededReason !== undefined) {
            ...
            const alreadyTracked = this.items.some(item => item.sessionId === id
                ...
                && item.status !== 'expired');
            if (!alreadyTracked) {
                void this.ingest(signalFromAuthoritativeHumanWait({ id, ...(cwd === undefined ? {} : { header: { cwd } }) }, authoritativeSnapshot.humanNeededReason, authoritativeSnapshot.humanNeededSeq));
            }
        }
        this.bumpRevision();
        this.syncSupervisor();
    ...
    onSessionEvent(sessionValue, eventValue, authoritativeSnapshot) {
        ...
        if (event.type === 'tool/call') {
            if (callId === undefined || !session.activeToolIds.has(callId)) {
                ...
                if (callId !== undefined)
                    session.activeToolIds.set(callId, observedToolName);
            }
        ...
        const signals = wasStalled && meaningful
            ...
            : [];
        signals.push(...signalsFromSessionEvent({ id, header: { ...(session.cwd ? { cwd: session.cwd } : {}), ...(typeof parentSession === 'string' ? { parentSession } : {}) } }, { type: event.type, ...(typeof event.seq === 'number' ? { seq: event.seq } : {}), ...(typeof event.time === 'number' ? { time: event.time } : {}), ...(event.ignorable === true ? { ignorable: true } : {}), data: asRecord(event.data) }, facts));
        ...
        if (humanWaitResolved || event.type === 'turn/end') {
            ...
            const state = event.type !== 'turn/end' ? 'running'
                : reason === 'completed' ? 'completed' : reason === 'aborted' ? 'aborted' : 'unknown';
            void this.ready.then(() => { if (!this.disposed)
                this.resolveHumanWait(id, now, state); });
        }
    ...
    resolveHumanWait(sessionId, now, taskState) {
        let changed = false;
        for (const item of this.items) {
            if (item.sessionId !== sessionId || item.taskState !== 'waiting-human'
                ...
                continue;
            if (!(item.reasonCodes ?? [item.reasonCode]).some(reason => reason === 'HUMAN_QUESTION_PENDING' || reason === 'HUMAN_APPROVAL_REQUIRED'))
                continue;
            item.status = 'recovered';
    ...
    onAgentError(payload) {
        ...
        this.ready.then(() => {
            if (!this.disposed)
                this.resolveHumanWait(id, now, 'failed');
        });
        this.bumpRevision();
        this.syncSupervisor();
    }
    ...
    mergeBundle(item, verdict, signal) {
        ...
        if (incomingTaskState !== undefined)
            item.taskState = incomingTaskState;
        if (signal.sessionId !== undefined && item.targetAvailable === undefined)
            item.targetAvailable = true;
    ...
    applySettings(value) {
        ...
        if (this.started) {
            this.resetLivenessTimer();
            this.resetHostProbeTimer();
            if (next.supervisorMode === 'experimental')
                this.ensureSupervisorStarted();
            else if (this.supervisorStarted)
                this.logger.warn?.(`${PLUGIN_NAME}: supervisorMode=off takes effect after restart because the experimental Supervisor is active`);
        }
        this.syncSupervisor();
    }
    resetLivenessTimer() {
    ...
    reconcileOrphans(now) {
        ...
        if (status.phase !== 'ready' || !status.authoritative || !status.verified)
            return false;
        ...
        for (const item of this.items) {
            ...
            if (!isActiveInboxStatus(item.status))
                continue;
            ...
            if (!Number.isFinite(orphanedAt) || now - orphanedAt < ORPHAN_GRACE_MS)
                continue;
            item.status = 'expired';
