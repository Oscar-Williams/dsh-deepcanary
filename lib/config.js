...
export const Config = Schema.object({
    ...
    supervisorMode: Schema.union(['off', 'experimental']).default('off').description('Persistent Supervisor lifecycle; experimental is an explicit engineering opt-in'),
});
const defaults = {
...
export function sanitizeConfigPatch(input) {
    ...
    if (input.openOnCritical !== undefined) {
        if (typeof input.openOnCritical !== 'boolean')
            throw new TypeError('openOnCritical must be boolean');
    ...
    if (input.supervisorMode !== undefined) {
        if (input.supervisorMode !== 'off' && input.supervisorMode !== 'experimental')
            throw new TypeError('supervisorMode must be off or experimental');
