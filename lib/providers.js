...
export function toolClassForName(name) {
...
export function toolCallIdOf(data) {
    ...
    if (message !== null && typeof message === 'object') {
        if ('source' in message && message.source !== null && typeof message.source === 'object'
            ...
            && 'callId' in message.source && typeof message.source.callId === 'string')
...
export function isMeaningfulSessionEvent(eventType, data = {}) {
    if (data.progress === true || data.meaningfulProgress === true)
        return true;
    if (data.progress === false || data.meaningfulProgress === false)
        return false;
 ...
 */
export function signalFromAuthoritativeHumanWait(session, reason, eventSeq, occurredAt = Date.now()) {
    const kind = reason === 'approval' ? 'HUMAN_APPROVAL_REQUIRED' : 'HUMAN_QUESTION_PENDING';
    const eventRef = `authoritative-human-wait:${eventSeq ?? 'snapshot'}`;
    return signal('session', kind, session, { type: eventRef, ...(eventSeq === undefined ? {} : { seq: eventSeq }), time: occurredAt }, undefined, evidence('session-event', 'runtime', eventRef, 'DSH authoritative session history reports an unresolved human interaction boundary.'), {}, 3, `${session.id}:human-needed`);
}
...
export function signalFromHostProbe(ok, detail, now = Date.now(), outageId) {
    if (ok)
        return undefined;
...
export function signalFromStall(session, facts, thresholdMs, now = Date.now()) {
    if (facts.turnState === 'terminal' || facts.turnState === 'unknown')
        return undefined;
    if (facts.waitingForHuman === true)
        return undefined;
    ...
    const effectiveThreshold = facts.activeToolCount !== undefined && facts.activeToolCount > 0
        ...
        : thresholdMs;
    if (meaningfulIdleMs < effectiveThreshold)
        return undefined;
...
export function signalFromHostRecovery(outageId, now = Date.now()) {
