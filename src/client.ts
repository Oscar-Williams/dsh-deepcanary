...
async function post(path: string, payload: Record<string, unknown>): Promise<Response | undefined> {
  try {
...
function selectControl<T extends string>(name: string, values: readonly T[], current: T): HTMLSelectElement {
...
function numberControl(name: string, current: number, min: number, max: number): HTMLInputElement {
...
function renderSettings(parent: HTMLElement, settings: ClientSettings): void {
  ...
  row.append(
    field('提醒级别', selectControl('notificationLevel', ['C1', 'C2', 'C3'], settings.notificationLevel)),
    field('每小时最多打断', numberControl('maxInterruptsPerHour', settings.maxInterruptsPerHour, 0, 10)),
  )
  form.append(row)
  const thresholds = el('div')
  thresholds.style.cssText = 'display:grid;grid-template-columns:1fr 1fr;gap:8px'
  thresholds.append(
    field('停滞阈值（分钟）', numberControl('longRunThresholdMinutes', settings.longRunThresholdMinutes, 1, 120)),
    field('相邻事件合并（秒）', numberControl('bundleWindowSeconds', settings.bundleWindowSeconds, 0, 900)),
  )
...
function render(snapshot: ClientSnapshot): void {
  ...
  const requestButton = button('启用浏览器通知', () => {
    if ('Notification' in globalThis) void Notification.requestPermission()
  })
  ...
  for (const item of snapshot.inbox.slice(0, 12)) {
    ...
    const addAction = (label: string, value: Record<string, unknown>): void => {
    ...
    if (item.sessionId) {
      const jump = button('跳转到 DSH', () => {
        void post('/dsh-deepcanary/action', { id: item.id, action: 'jump' }).then(async response => {
          ...
          const result = await response.json() as JumpResult
