...
const outcomeSources = new Set<OutcomeSource>(['real', 'controlled', 'replay'])
const outcomeFeedback = new Set<OutcomeFeedback>(['unrated', 'useful', 'not-useful'])
const outcomeLaterOutcomes = new Set<OutcomeLaterOutcome>(['unknown', 'continued', 'completed', 'failed', 'aborted', 'recovered', 'user-stopped'])
const outcomeLatencyBuckets = new Set<OutcomeLatencyBucket>(['unknown', 'not-opened', 'under-1m', '1-5m', '5-15m', 'over-15m'])
const outcomeReviewFlags = new Set<OutcomeReviewFlag>([
  'missed-human-needed',
  'false-stall',
  'wrong-level',
...
const attentionLevels = new Set<AttentionLevel>(['C0', 'C1', 'C2', 'C3'])
const attentionActions = new Set<AttentionAction>(['IGNORE', 'INBOX', 'DIGEST', 'INTERRUPT', 'ESCALATE'])
const evidenceAuthorities = new Set<EvidenceAuthority>(['host', 'runtime', 'derived', 'heuristic'])
...
export function normalizeOutcomeInput(value: unknown): OutcomeReceiptInput {
  if (value === null || typeof value !== 'object') throw new TypeError('outcome payload must be an object')
  ...
  for (const key of Object.keys(raw)) if (!allowed.has(key)) throw new TypeError(`unsupported outcome field: ${key}`)

  if (!isOutcomeSource(raw.source)) throw new TypeError('outcome.source must be real, controlled, or replay')
  ...
  const booleans = ['opened', 'acknowledged', 'snoozed', 'muted', 'recoveredBeforeOpen'] as const
  for (const key of booleans) {
    if (raw[key] !== undefined && typeof raw[key] !== 'boolean') throw new TypeError(`outcome.${key} must be boolean`)
  }
  if (raw.feedback !== undefined && !isOutcomeFeedback(raw.feedback)) throw new TypeError('outcome.feedback is unsupported')
  if (raw.laterOutcome !== undefined && !isOutcomeLaterOutcome(raw.laterOutcome)) throw new TypeError('outcome.laterOutcome is unsupported')
  if (raw.latencyBucket !== undefined && !isOutcomeLatencyBucket(raw.latencyBucket)) throw new TypeError('outcome.latencyBucket is unsupported')

  let reviewFlags: OutcomeReviewFlag[] | undefined
...
export function normalizeOutcomeDeleteFilter(value: unknown): OutcomeDeleteFilter {
  if (value === null || typeof value !== 'object') throw new TypeError('outcome delete filter must be an object')
  const raw = value as Record<string, unknown>
  const allowed = new Set(['source', 'trialId', 'before'])
  for (const key of Object.keys(raw)) if (!allowed.has(key)) throw new TypeError(`unsupported outcome delete field: ${key}`)
  if (raw.source !== undefined && !isOutcomeSource(raw.source)) throw new TypeError('outcome delete source is unsupported')
...
export function eventClassForReason(reasonCode: ReasonCode): OutcomeEventClass {
  switch (reasonCode) {
...
export function strongestEvidenceAuthority(item: InboxItem): EvidenceAuthority {
  const authorityRank: Record<EvidenceAuthority, number> = { heuristic: 0, derived: 1, runtime: 2, host: 3 }
  return item.evidence.reduce<EvidenceAuthority>((strongest, evidence) => authorityRank[evidence.authority] > authorityRank[strongest] ? evidence.authority : strongest, 'heuristic')
}

export function buildOutcomeReceipt(item: InboxItem, input: OutcomeReceiptInput, previous?: OutcomeReceipt, recordedAt = new Date().toISOString()): OutcomeReceipt {
...
export class OutcomeStore {
  ...
  async load(): Promise<OutcomeReceipt[]> {
    try {
      const raw = JSON.parse(await readFile(this.file, 'utf8')) as Partial<PersistedOutcomeState>
      if (raw.schemaVersion !== OUTCOME_RECEIPT_SCHEMA_VERSION || !Array.isArray(raw.receipts)) return []
  ...
  async save(receipts: readonly OutcomeReceipt[]): Promise<void> {
    const payload: PersistedOutcomeState = {
      schemaVersion: OUTCOME_RECEIPT_SCHEMA_VERSION,
...
export function isOutcomeReceipt(value: unknown): value is OutcomeReceipt {
  if (value === null || typeof value !== 'object') return false
  if (Object.keys(value).some(key => !outcomeReceiptFields.has(key))) return false
  const receipt = value as Partial<OutcomeReceipt>
...
function cloneOutcomeReceipt(receipt: OutcomeReceipt): OutcomeReceipt {
  return { ...receipt, reviewFlags: [...receipt.reviewFlags] }
}

function isOutcomeSource(value: unknown): value is OutcomeSource {
  return typeof value === 'string' && outcomeSources.has(value as OutcomeSource)
}

function isOutcomeFeedback(value: unknown): value is OutcomeFeedback {
  return typeof value === 'string' && outcomeFeedback.has(value as OutcomeFeedback)
}

function isOutcomeLaterOutcome(value: unknown): value is OutcomeLaterOutcome {
  return typeof value === 'string' && outcomeLaterOutcomes.has(value as OutcomeLaterOutcome)
}

function isOutcomeLatencyBucket(value: unknown): value is OutcomeLatencyBucket {
  return typeof value === 'string' && outcomeLatencyBuckets.has(value as OutcomeLatencyBucket)
}

function isOutcomeReviewFlag(value: unknown): value is OutcomeReviewFlag {
  return typeof value === 'string' && outcomeReviewFlags.has(value as OutcomeReviewFlag)
}

function isOutcomeEventClass(value: unknown): value is OutcomeEventClass {
...
function isReasonCode(value: unknown): value is ReasonCode {
  return typeof value === 'string' && reasonCodes.has(value as ReasonCode)
}

function isAttentionLevel(value: unknown): value is AttentionLevel {
  return typeof value === 'string' && attentionLevels.has(value as AttentionLevel)
}

function isAttentionAction(value: unknown): value is AttentionAction {
  return typeof value === 'string' && attentionActions.has(value as AttentionAction)
}

function isEvidenceAuthority(value: unknown): value is EvidenceAuthority {
  return typeof value === 'string' && evidenceAuthorities.has(value as EvidenceAuthority)
}

function isReviewFlagArray(value: unknown): value is OutcomeReviewFlag[] {
  return Array.isArray(value) && value.length <= 7 && value.every(isOutcomeReviewFlag)
}

function boundedPrintable(value: string, max: number): boolean {
...
interface PersistedOutcomeState {
