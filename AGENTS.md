...
- Keep two explicit runtime lanes: the historical RC.2 receipt uses official alpha.2 at its pinned commit; current development and new compatibility tests use official alpha.4 at tag `dsh-v0.1.2-alpha.4`, commit `4e84901e6471b79ec0338099867ebb4606d12bb5`. Never relabel one lane's evidence as the other.
...
For current development end-to-end checks, use the official alpha.4 checkout documented in `docs/compatibility.md`, verify `dsh --version` is `0.1.2-alpha.4`, install the current built package into the isolated web profile, run `dsh web`, and check HTTP 200 on the DeepCanary health route. Use the alpha.2 checkout only when reproducing the historical RC.2 receipt.
